...
namespace patch {
    // init at exedit load
    // 編集プロジェクトの保存時、レイヤー情報を保存する基準を変える
    ...
    // 編集プロジェクトの保存時にシーン設定の一部情報（グリッド設定など）が欠けるのを修正

    inline class aup_save_t {

        bool enabled = true;
        bool enabled_i;

        inline static const char key[] = "aup_save";
...
#endif // ifdef PATCH_SWITCH_AUP_SAVE
